...
class BorrowingSerializer(serializers.Serializer):
    user = serializers.SlugRelatedField(
        queryset=User.objects.all(), many=False, slug_field="id"
    )
    book = serializers.SlugRelatedField(
        queryset=Book.objects.all(), many=False, slug_field="title"
    )
...
class ReturnBookSerializer(serializers.Serializer):
    returned_borrowing = serializers.SlugRelatedField(
        queryset=Borrowing.objects.all(), many=False, slug_field="id"
    )
